...
currentAccount: any;
    ...
    loadAll() {
        if (this.currentSearch) {
            this.orderActivityService.search({
                page: this.page - 1,
    ...
    transition() {
        this.router.navigate(['/order-activity'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    clear() {
        this.page = 0;
        this.currentSearch = '';
        this.router.navigate(['/order-activity', {
            page: this.page,
            sort: this.predicate + ',' + (this.reverse ? 'asc' : 'desc')
        }]);
        this.loadAll();
    ...
    trackId(index: number, item: OrderActivity) {
        return item.id;
    }
    registerChangeInOrderActivities() {
        this.eventSubscriber = this.eventManager.subscribe('orderActivityListModification', (response) => this.loadAll());
    }
