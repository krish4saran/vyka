...
currentAccount: any;
    ...
    loadAll() {
        if (this.currentSearch) {
            this.packageOrderService.search({
                page: this.page - 1,
    ...
    transition() {
        this.router.navigate(['/package-order'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    clear() {
        this.page = 0;
        this.currentSearch = '';
        this.router.navigate(['/package-order', {
            page: this.page,
            sort: this.predicate + ',' + (this.reverse ? 'asc' : 'desc')
        }]);
        this.loadAll();
    ...
    trackId(index: number, item: PackageOrder) {
        return item.id;
    }
    registerChangeInPackageOrders() {
        this.eventSubscriber = this.eventManager.subscribe('packageOrderListModification', (response) => this.loadAll());
    }
